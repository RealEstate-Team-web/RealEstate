...
const UserManagement = () => {
  ...
  if (loading) {
    return <div className="py-20 text-center text-[#6B7280] dark:text-slate-400">Loading users…</div>;
  }
  ...
  return (
    <div className="space-y-5 font-sans">
      ...
      {error && (
        <div
          role="alert"
          className="rounded-md bg-[#FBE9E8] text-[#B23B36] text-[13px] px-4 py-3 dark:bg-rose-500/10 dark:text-rose-300"
        >
          {error}
        </div>
      )}
      {success && (
        <div
          role="status"
          aria-live="polite"
          className="rounded-md bg-[#E6F4EC] text-[#2F7A55] text-[13px] px-4 py-3 dark:bg-emerald-500/10 dark:text-emerald-300"
        >
          {success}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          value={activeUsers}
          icon={CheckCircle}
          iconBg="bg-[#E6F4EC] text-[#4FAF83] dark:bg-emerald-500/10 dark:text-emerald-300"
        ...
        <KpiCard
          ...
          icon={Ban}
          iconBg="bg-[#FBE9E8] text-[#D96B67] dark:bg-rose-500/10 dark:text-rose-300"
        ...
        <KpiCard
          ...
          value={adminAccounts}
          icon={ShieldCheck}
          iconBg="bg-[#E6F4EC] text-[#1D6FD3] dark:bg-emerald-500/10 dark:text-blue-300"
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-lg shadow-[0_2px_8px_rgba(15,23,42,0.06)] overflow-hidden dark:bg-[#111827] dark:border-slate-800">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[13px] text-[#111827] min-w-[820px] dark:text-white">
            <thead>
            ...
            <tbody className="divide-y divide-[#E5E7EB] dark:divide-slate-800">
              {users.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-10 text-center text-[13px] text-[#6B7280] dark:text-slate-400">
                    No users found.
                  </td>
                </tr>
              ) : (
                users.map((u) => {
                  ...
                  return (
                    <tr
                      key={u.id}
                      className={`h-[50px] hover:bg-[#F9FAFB] transition-colors dark:hover:bg-slate-800/60 ${isHighlighted ? 'bg-[#E7F0FB]/60 dark:bg-blue-500/10' : ''}`}
                    >
                      <td className="py-0 px-4 text-[#374151] whitespace-nowrap dark:text-slate-200">{number}</td>
                      <td className="py-0 px-4 font-medium truncate dark:text-white">{name}</td>
                      <td className="py-0 px-4 text-[#374151] truncate dark:text-slate-200">{u.email}</td>
                      <td className="py-0 px-4 text-[#374151] whitespace-nowrap dark:text-slate-200">
                        {u.phone || '—'}
                      </td>
                      <td className="py-0 px-4 capitalize text-[#374151] dark:text-slate-200">{u.role}</td>
                      ...
                      <td className="py-0 px-4">
                        <div className="flex items-center gap-2 whitespace-nowrap">
                          {confirmId === u.id ? (
                            <>
                              <span className="text-[12px] text-[#B23B36] dark:text-rose-300">
        ...
        {pagination && (pagination.totalPages > 1 || pagination.hasNextPage || pagination.hasPrevPage) && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-[#E5E7EB] text-[13px] text-[#374151] dark:border-slate-800 dark:text-slate-200">
            <span>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1 h-[32px] px-3 rounded-md bg-[#edf2fa] border border-[#d6deeb] text-[13px] font-medium text-[#374151] hover:bg-[#F3F4F8] transition-colors disabled:opacity-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-700"
              >
                <ChevronLeft size={15} />
                Previous
              </button>
