  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-4">
        ...
        {/* Filter Button - Darker Blue (#1d4ed8 / blue-700) */}
        <button className="flex items-center space-x-2 bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-md shadow-blue-700/20 transition cursor-pointer">
          <Filter size={15} />
          <span>Filters</span>
          <span className="bg-blue-900 text-white text-[10px] px-1.5 py-0.5 rounded-full ml-1">3</span>
        </button>
          ...
          <div className="bg-slate-100 p-1 rounded-xl flex items-center space-x-1">
            <button
              ...
              className={`p-1.5 rounded-lg text-xs font-medium transition cursor-pointer ${
                viewMode === 'split' ? 'bg-white text-blue-700 shadow-xs' : 'text-slate-500 hover:text-slate-900'
              }`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* OpenStreetMap Map Provider Container (Left 5 Cols when split) */}
        {viewMode === 'split' && (
          <div className="lg:col-span-5 bg-white border border-slate-200/80 rounded-2xl overflow-hidden min-h-[520px] relative shadow-xs flex flex-col justify-between p-3">
            {/* Map Header Overlay */}
            <div className="flex items-center justify-between bg-slate-900 text-white p-3 rounded-xl mb-2 z-10 shadow-md">
              <span className="text-xs font-bold flex items-center gap-1.5">
                <MapPin size={15} className="text-blue-400" /> OpenStreetMap Provider (Addis Ababa)
              </span>
              <span className="text-[10px] font-semibold bg-blue-800/80 text-blue-100 px-2 py-0.5 rounded">
                Live Interactive
              </span>
            </div>

            {/* Embedded OpenStreetMap Iframe Centered on Addis Ababa */}
            <div className="relative flex-1 rounded-xl overflow-hidden border border-slate-200">
              <iframe
                ...
                loading="lazy"
                src="https://www.openstreetmap.org/export/embed.html?bbox=38.7200%2C8.9700%2C38.8300%2C9.0500&amp;layer=mapnik&amp;marker=9.0192%2C38.7525"
              ></iframe>

              {/* Price Cluster Chips Floating Overlay */}
              <div className="absolute top-3 left-3 z-10 flex flex-col space-y-1.5">
                {demoProperties.slice(0, 3).map((p) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full text-[11px] font-extrabold shadow-lg transition cursor-pointer border ${
                      p.id === activePropertyId
                        ? 'bg-blue-800 text-white border-white scale-105'
                        : 'bg-white/90 backdrop-blur-xs text-slate-900 border-slate-300 hover:bg-blue-700 hover:text-white'
                    }`}
                  >
                    📍 {p.location.split(',')[0]}: {p.price}
                  </button>
                ))}
              </div>
            </div>

            {/* Active Property Footer Preview */}
            <div className="mt-2 bg-slate-50 border border-slate-200 rounded-xl p-2.5 flex items-center space-x-3">
              <img
                src={demoProperties.find((p) => p.id === activePropertyId)?.img}
                alt="Selected Property"
                className="w-12 h-12 rounded-lg object-cover"
              />
              <div className="flex-1 min-w-0">
                <h4 className="text-xs font-bold text-slate-900 truncate">
                  {demoProperties.find((p) => p.id === activePropertyId)?.title}
                </h4>
                <p className="text-[10px] text-slate-500">
                  {demoProperties.find((p) => p.id === activePropertyId)?.location}
                </p>
                <p className="text-xs font-extrabold text-blue-700 mt-0.5">
                  {demoProperties.find((p) => p.id === activePropertyId)?.price}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {demoProperties.map((prop) => (
              <div
                ...
                className={`bg-white border rounded-2xl overflow-hidden shadow-xs hover:shadow-lg transition-all duration-200 flex flex-col justify-between group cursor-pointer ${
                  prop.id === activePropertyId ? 'border-blue-700 ring-2 ring-blue-700/20' : 'border-slate-200/80'
                }`}
              >
                <div>
                  {/* Image & Badges */}
                  <div className="relative h-44 overflow-hidden bg-slate-100">
                    ...
                    <button className="absolute top-3 right-3 p-2 bg-white/90 hover:bg-white text-slate-600 hover:text-rose-500 rounded-full backdrop-blur-xs transition cursor-pointer shadow-xs">
                      <Heart size={16} />
                    </button>
                  </div>
                ...
                {/* Price & Action Button (Darker Blue) */}
