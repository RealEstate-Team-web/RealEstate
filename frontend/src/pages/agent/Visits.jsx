...
import { getAgentVisitRequests, approveVisit, rejectVisit } from '../../services/visit.service';
...
export const AgentVisits = () => {
  ...
  const loadVisits = useCallback(
    ...
    [currentPage, statusFilter, debouncedSearch, sortBy, itemsPerPage]
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Escape') {
        if (approveConfirmTarget && !processing) setApproveConfirmTarget(null);
        if (rejectConfirmTarget && !processing) setRejectConfirmTarget(null);
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [approveConfirmTarget, rejectConfirmTarget, processing]);
  ...
  const handleConfirmApprove = async () => {
    ...
    setVisits((prev) =>
      prev.map((v) => (v.id === targetId ? { ...v, status: 'approved' } : v))
    );

    try {
      await approveVisit(targetId);
      showToast(`Visit request for "${approveConfirmTarget.propertyTitle}" approved`);
      loadVisits();
      setApproveConfirmTarget(null);
    } catch (err) {
      console.error('Failed to approve visit:', err);
      showToast(err.response?.data?.message || 'Failed to approve visit');
      setApproveConfirmTarget(null);
      loadVisits();
    } finally {
      setProcessing(false);
  ...
  const handleConfirmReject = async () => {
    ...
    const targetId = rejectConfirmTarget.id;
    setProcessing(true);
    ...
    try {
      await rejectVisit(targetId);
      showToast(`Visit request for "${rejectConfirmTarget.propertyTitle}" rejected`);
      loadVisits();
      setRejectConfirmTarget(null);
    } catch (err) {
      console.error('Failed to reject visit:', err);
      showToast(err.response?.data?.message || 'Failed to reject visit');
      setRejectConfirmTarget(null);
      loadVisits();
    } finally {
      setProcessing(false);
                  ...
                  {(v.buyerFirstName || v.buyerLastName) && (
                    <div className="flex items-center space-x-2.5 pt-1 text-xs">
                      {v.buyerAvatar ? (
                        <img
                          src={v.buyerAvatar}
                          alt={`${v.buyerFirstName} ${v.buyerLastName}`}
