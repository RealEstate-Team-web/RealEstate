...
const ProfilePhotoUpload = ({ onChange, error }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-[13px] font-semibold text-[#101820] dark:text-white">Profile Photo (optional)</span>
      <div className="flex items-center gap-4">
        <button
          type="button"
          onClick={() => inputRef.current?.click()}
          aria-label="Upload profile photo"
          className="relative flex h-[50px] w-[50px] shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#E5E7E8] dark:bg-slate-800"
        >
          {preview ? (
            <img
              src={preview}
