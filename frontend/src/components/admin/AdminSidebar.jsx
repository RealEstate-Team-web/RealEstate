      ...
      <div>
        <div className="flex items-center justify-between h-[68px] px-4 border-b border-white/5">
          <div className="flex items-center space-x-2.5">
            <div className="w-9 h-9 rounded-lg bg-[#4A9FF5]/15 text-[#5AA9F8] flex items-center justify-center">
              <Home size={20} className="stroke-[2.5]" />
            </div>
            <div className="leading-tight">
              <h1 className="font-bold text-white text-[17px] tracking-tight">NestHome</h1>
          ...
          {navItems.map((item) => (
            ...
            >
              <div className="flex items-center space-x-3">
                <item.icon size={19} className="transition-transform duration-200 group-hover:scale-105" />
                <span>{item.label}</span>
              </div>
              <div className="flex items-center">
                {item.badge && (
                  <span className={`px-1.5 py-0.5 text-[11px] font-bold text-white rounded-full ${item.badgeColor}`}>
                    {item.badge}
                  </span>
                )}
                {item.dot && <span className={`w-2 h-2 rounded-full ${item.badgeColor}`} />}
                {item.pill && (
                  <span className={`px-2 py-0.5 text-[10px] font-semibold text-white rounded-full ${item.pillColor}`}>
                    {item.pill}
                  </span>
                )}
              </div>
