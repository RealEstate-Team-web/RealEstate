const KpiCard = ({ title, value, indicator, icon: Icon, iconBg = 'bg-[#E7F0FB] text-[#4A9FF5] dark:bg-blue-500/10 dark:text-blue-300' }) => (
  <div className="bg-white border border-[#E5E7EB] rounded-lg p-4 shadow-[0_2px_8px_rgba(15,23,42,0.06)] flex flex-col justify-between min-h-[118px] hover:shadow-[0_4px_14px_rgba(15,23,42,0.08)] transition-shadow dark:bg-[#111827] dark:border-slate-800">
    <div className="flex items-start justify-between">
      <p className="text-[13px] font-medium text-[#6B7280] dark:text-slate-400">{title}</p>
      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${iconBg}`}>
        {Icon && <Icon size={17} />}
      </div>
    </div>
    <h3 className="text-[28px] font-bold text-[#111827] leading-none mt-2 dark:text-white">{value}</h3>
